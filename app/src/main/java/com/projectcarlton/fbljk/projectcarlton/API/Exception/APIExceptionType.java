package com.projectcarlton.fbljk.projectcarlton.API.Exception;

public class APIExceptionType {

    public static final int AUTHENTICATION_FAILED           = 1;
    public static final int MALFORMED_INPUT                 = 2;
    public static final int UNKNOWN_METHOD                  = 3;
    public static final int DB_CONNECTION_NOT_ETABLISHED    = 4;
    public static final int UNKNOWN_USER                    = 5;
    public static final int WRONG_PASSWORD                  = 6;
    public static final int EXISTING_EMAIL                  = 7;
    public static final int EXISTING_USERNAME               = 8;
    public static final int INVALID_EMAIL                   = 9;
    public static final int NO_ACTIVATED_ACCOUNT            = 10;
    public static final int UNKNOWN_ERROR                   = 11;

}